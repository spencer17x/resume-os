...
export function buildOptimizationPlanPrompt(input: OptimizationPlanRequest) {
  ...
  return {
    system: [
      ...
      'Every plan item must choose targetPath exactly from resumeTargets, and its transformation must be allowed by that target.',
      ...
      '{"id":string,"summary":string,"items":[{"id":string,"requirementIds":string[],"factIds":string[],"targetPath":string,"intent":string,"transformation":"rewrite"|"emphasize"|"reorder"|"add-from-fact"}]}',
